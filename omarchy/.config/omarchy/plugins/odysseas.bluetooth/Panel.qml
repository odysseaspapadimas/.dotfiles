...
Panel {
  id: root
  moduleName: "odysseas.bluetooth"
  ipcTarget: "odysseas.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  Component.onCompleted: refreshNothingBattery()
  ...
  // Per-component Nothing battery readings from something-x. Each value has
  // its own timestamp because the charging case is only reported while it is
  // awake. Old readings disappear rather than pretending to still be current.
  property string nothingBatteryAddress: ""
  property int nothingLeftBattery: -1
  property int nothingRightBattery: -1
  property int nothingCaseBattery: -1
  property double nothingLeftReportedAt: 0
  property double nothingRightReportedAt: 0
  property double nothingCaseReportedAt: 0
  property double batteryClock: Date.now()
  readonly property int nothingBatteryMaxAgeMs: 90000

  function isNothingDevice(device) {
    if (!device) return false
    var label = deviceLabel(device).toLowerCase()
    var address = Model.normalizedAddress(device.address)
    return label.indexOf("nothing ear") !== -1 || address === "3cb0edb203a0"
  }

  function freshBattery(value, reportedAt) {
    return value >= 0 && reportedAt > 0 && batteryClock - reportedAt <= nothingBatteryMaxAgeMs
  }

  function nothingBatteryText(device) {
    if (!device || !isNothingDevice(device)
        || Model.normalizedAddress(device.address) !== Model.normalizedAddress(nothingBatteryAddress)) return ""
    var parts = []
    if (freshBattery(nothingLeftBattery, nothingLeftReportedAt)) parts.push("L: " + nothingLeftBattery + "%")
    if (freshBattery(nothingRightBattery, nothingRightReportedAt)) parts.push("R: " + nothingRightBattery + "%")
    if (freshBattery(nothingCaseBattery, nothingCaseReportedAt)) parts.push("C: " + nothingCaseBattery + "%")
    return parts.join("  ")
  }

  function refreshNothingBattery() {
    if (nothingBatteryProcess.running) return
    // This is the paired Nothing Ear (a). bluetoothctl gates the relatively
    // expensive RFCOMM query so disconnected earbuds return immediately.
    nothingBatteryAddress = "3C:B0:ED:B2:03:A0"
    nothingBatteryProcess.command = [
      "sh", "-c",
      "bluetoothctl info 3C:B0:ED:B2:03:A0 | grep -q 'Connected: yes' && exec something-x --battery --device 3C:B0:ED:B2:03:A0"
    ]
    nothingBatteryProcess.running = true
  }
  ...
  Process {
    id: nothingBatteryProcess
    stdout: StdioCollector { id: nothingBatteryOutput; waitForEnd: true }
    onExited: function(exitCode) {
      var output = String(nothingBatteryOutput.text || "")
      if (exitCode !== 0) return
      var now = Date.now()
      var match = /Left:\s*(\d+)%/i.exec(output)
      if (match) { root.nothingLeftBattery = Number(match[1]); root.nothingLeftReportedAt = now }
      match = /Right:\s*(\d+)%/i.exec(output)
      if (match) { root.nothingRightBattery = Number(match[1]); root.nothingRightReportedAt = now }
      match = /Case:\s*(\d+)%/i.exec(output)
      if (match) { root.nothingCaseBattery = Number(match[1]); root.nothingCaseReportedAt = now }
      root.batteryClock = now
    }
  }
  ...
  Timer {
    ...
    triggeredOnStart: true
    running: root.opened
    onTriggered: root.refreshNothingBattery()
  }
  ...
  Timer {
    ...
    repeat: true
    running: root.opened
    onTriggered: root.batteryClock = Date.now()
  }
  ...
  component DeviceRow: CursorSurface {
    ...
    readonly property string statusText: {
      ...
      if (isConnected) {
        var preciseBattery = root.nothingBatteryText(dev)
        if (preciseBattery !== "") return preciseBattery
        if (root.isNothingDevice(dev)) return ""
