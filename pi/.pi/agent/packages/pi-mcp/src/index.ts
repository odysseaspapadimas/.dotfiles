...
export default function opencodeMcpExtension(pi: ExtensionAPI) {
  ...
  async function ensureManager(ctx: ExtensionContext) {
    ...
    manager = new McpManager({
      ...
      // Pi runs on a remote host: never open its browser. The command callback
      // prints the URL so the user can open it on their own machine.
      openAuthorizationUrl: (url) => showCommandMessage(pi, "Open MCP OAuth URL", url),
  ...
  function updateMcpStatus(ctx: ExtensionContext) {
    ...
    if (ctx.hasUI) {
      ctx.ui.setStatus("mcp", count > 0 ? ctx.ui.theme.fg("dim", `mcp ${count}`) : undefined);
    }
  }
