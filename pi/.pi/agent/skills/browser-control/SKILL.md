...
This Ubuntu host reaches Browser Control on the user's Mac through portd's
loopback-only SSH reverse forward. Run normal `browser-control` commands here;
port `19989` is already forwarded to the Mac relay. Do not start `serve` on
Ubuntu.

The controlled Chrome runs on the Mac. Remote development servers therefore
must be opened through their **Mac-local portd mapping**, not their Ubuntu port.
Read the mapping before navigation:

```bash
curl -fsS http://127.0.0.1:43117/api/status | jq '.tunnels[] | select(.state == "active") | {remote_port, local_port, label}'
```

For example, if Ubuntu `5173` maps to Mac `5174`, navigate the controlled page
to `http://127.0.0.1:5174`. Verify `browser-control status` reports the extension
connected before acting. If the relay is unavailable, report it; never start a
second Ubuntu relay.
