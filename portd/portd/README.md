...
- `portd` runs on the Mac and owns one system OpenSSH ControlMaster connection.
  It discovers Ubuntu listeners and makes them available on Mac loopback, and it
  discovers Mac listeners that can be persistently exposed on Ubuntu loopback.
- `ports` is a Ratatui client for both directions. It works on the Mac and on the
...
- **Mac → Ubuntu** makes Ubuntu services available on Mac loopback. These ports
  are automatically forwarded through SSH.
- **Ubuntu → Mac** exposes selected Mac-local services on Ubuntu loopback. Mac
  IPv4 loopback listeners are discovered automatically, but are never exposed
...
port. In the Ubuntu → Mac view, enter either `PORT` for the same port at both
ends or `UBUNTU_PORT:MAC_PORT` for distinct ports.

In the Mac → Ubuntu view, press `l` to add, edit, or clear a persistent label.
Labels are stored by the Mac daemon and appear in both the local and remote TUI.
...
Mac-local services are managed persistently from the Ubuntu → Mac view. For
example, an enabled `5037:5037` mapping gives Ubuntu's normal ADB client secure
access to the Mac ADB server without changing `ADB_SERVER_SOCKET`:

```text
Ubuntu 127.0.0.1:5037 -> SSH -> Mac 127.0.0.1:5037
```
...
helper resolves portd's current Mac-local mapping, so it also works when a Mac
port differs from the Ubuntu service port. Device mappings remain ADB state and
